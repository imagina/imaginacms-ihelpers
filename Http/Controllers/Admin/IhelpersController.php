        ...
        try {
            ...
            <priority>1.00</priority>
                        ...
                        if (!empty($configEntities['relation'])) {
                            foreach ($configEntities['relation'] as $relationEntity) {
                                $relation = $this->queryRelationSiteMap($relationEntity, $key->id);
                                ...
                                foreach ($relation as $key2) {
                                    ...
                                    $xml .= '<url>
                  <loc>' . str_replace($relationEntity['replaceInUrl'], $key2->slug, $relationEntity['url']) . '</loc>
                  <lastmod>' . date("Y-m-d H:i:s") . '</lastmod>
                  <priority>1.00</priority>
                  </url>';
                                    $counter++;
                                    $routesArray = array_merge($routesArray, array(['title' => $key2->title, 'type' => $relationEntity['name'], 'route' => str_replace($relationEntity['replaceInUrl'], $key2->slug, $relationEntity['url'])]));
                                    if (!empty($relationEntity['relation'])) {
                                        $relation2 = $this->queryRelationSiteMap($relationEntity['relation'], $key2->id);
                                        $html .= '<li class="lhead ml-40" ><a href="' . str_replace($relationEntity['replaceInUrl'], $key2->slug, $relationEntity['url']) . '/">' . $key2->title . '</a> / <span class="lcount">' . count($relation2) . ' pages</span></li>';
                                        /* Test
                                        if($key2->id==131 || $key2->id=="131"){
                                        $entidad=app('\Modules\Icommerce\Entities\Product');
                                        dd($key2->id,$relationEntity['relation'],$relation2,$entidad->with(['category','categories','parent','tags','manufacturer','product_discounts'])->whereStatus(Status::ENABLED)->where('date_available','<=',date('Y-m-d'))->take(10)->where('category_id',131)->orderBy('created_at', 'DESC')->get());
                                      }*/
                                        foreach ($relation2 as $key3) {
                                            ...
                                            $xml .= '<url>
                    <loc>' . str_replace($relationEntity['relation']['replaceInUrl'], $key3->slug, $relationEntity['relation']['url']) . '</loc>
                    <lastmod>' . date("Y-m-d H:i:s") . '</lastmod>
                    <priority>1.00</priority>
                    </url>';
                                            $html .= '<li class="lpage ml-60" ><a href="' . str_replace($relationEntity['relation']['replaceInUrl'], $key3->slug, $relationEntity['relation']['url']) . '/">' . $key3->title . '</a></li>';
                                            $counter++;
                                            $routesArray = array_merge($routesArray, array(['title' => $key3->title, 'type' => $relationEntity['relation']['name'], 'route' => str_replace($relationEntity['relation']['replaceInUrl'], $key3->slug, $relationEntity['relation']['url'])]));
                                        }//foreach relation
                                    }//relation
                                    else {
                                        $html .= '<li class="lpage ml-40" ><a href="' . str_replace($relationEntity['replaceInUrl'], $key2->slug, $relationEntity['url']) . '/">' . $key2->title . '</a></li>';
                                    }
                                }//foreach relation
                                //dd($relationEntity);
                            }//foreach($configEntities['relation'] as $relationEntity){
